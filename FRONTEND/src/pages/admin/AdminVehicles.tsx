...
export default function AdminVehicles() {
  ...
  const emptyHint = useMemo(() => {
    if (search.trim().length > 0) {
      return `No vehicles found for "${search}".`;
    }
    return 'No vehicles yet. Add your first vehicle listing above.';
  }, [search]);
  ...
  const loadVehicles = async (term = search) => {
    ...
    setError(null);
    try {
      const response = await listAdminVehicles(term);
  ...
  const startEdit = (vehicle: AdminVehicle) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.type.trim() || !form.image.trim() || !form.fuel.trim() || !form.transmission.trim()) {
      setError('Please fill all required vehicle fields before saving.');
      return;
    }

    if (!Number.isFinite(pricePerDay) || !Number.isFinite(seats) || pricePerDay <= 0 || seats <= 0) {
      setError('Price per day and seats must be positive numbers.');
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await updateAdminVehicle(editingId, normalized);
      } else {
  ...
  const handleDelete = async (vehicle: AdminVehicle) => {
    if (!window.confirm(`Delete ${vehicle.name}? This action cannot be undone.`)) {
      return;
    }
  ...
  const handleApproval = async (vehicleId: string, approval: AdminVehicle['approval']) => {
    setBusyId(vehicleId);
    setError(null);

    try {
      await setVehicleApproval(vehicleId, approval);
      await loadVehicles(search);
    } catch (err) {
  ...
  const handleAvailability = async (vehicle: AdminVehicle) => {
    setBusyId(vehicle.id);
    setError(null);

    try {
      await setVehicleAvailability(vehicle.id, vehicle.status === 'available' ? 'unavailable' : 'available');
      await loadVehicles(search);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unable to update availability';
      setError(message);
      setBusyId(null);
    }
  };
  ...
  return (
    <section className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className={`${current.subtext} text-sm sm:text-base`}>
            Add/edit vehicles, approve or reject listings, and control availability.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button
            type="button"
            onClick={() => void loadVehicles(search)}
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          {editingId && (
            <button
              type="button"
              onClick={resetForm}
              className={`inline-flex items-center gap-1 rounded-xl border ${current.card} px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em]`}
            >
          ...
          {[
            ...
            <div key={field.key} className="space-y-1">
              ...
              <input
                ...
                value={form[field.key as keyof VehicleForm]}
                onChange={(event) =>
                  setForm((prev) => ({
                    ...prev,
      ...
      {!loading && !error && vehicles.length === 0 && (
        <AdminEmptyState title="No Vehicles Found" hint={emptyHint} />
      )}

      {!loading && !error && vehicles.length > 0 && (
        <AdminTable
          columns={['Vehicle', 'Specs', 'Approval', 'Availability', 'Updated', 'Actions']}
              ...
              <tr key={vehicle.id} className="border-b border-white/5 align-top">
                <td className="px-5 py-4">
                  <div className="flex items-start gap-3">
                    <img
                      src={vehicle.image}
                      alt={vehicle.name}
                      className="w-20 h-14 rounded-xl object-cover border border-white/10"
                      referrerPolicy="no-referrer"
                    />
                    <div>
                      <p className="font-bold text-sm">{vehicle.name}</p>
                      <p className="text-[11px] opacity-50">{vehicle.id}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-5 py-4 text-xs">
                  ...
                  <p className="opacity-60 mt-1">
                    ${vehicle.pricePerDay}/day | {vehicle.seats} seats
                  </p>
                ...
                <td className="px-5 py-4">
                  <span
                    className={`inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.16em] ${approvalBadge(vehicle.approval)}`}
                  >
                    {vehicle.approval}
                  </span>
                </td>
                ...
                <td className="px-5 py-4">
                  <span
                    className={`inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.16em] ${availabilityBadge(vehicle.status)}`}
                  >
                    {vehicle.status}
                  </span>
                </td>
                ...
                <td className="px-5 py-4">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      type="button"
                      disabled={isBusy}
                      onClick={() => void handleApproval(vehicle.id, 'approved')}
                      className="inline-flex items-center gap-1 rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-200 disabled:opacity-60"
                    >
                    ...
                    <button
                      type="button"
                      disabled={isBusy}
                      onClick={() => void handleApproval(vehicle.id, 'rejected')}
                    ...
                    <button
                      type="button"
                      disabled={isBusy}
                      onClick={() => void handleAvailability(vehicle)}
                      className="inline-flex items-center gap-1 rounded-xl border border-blue-400/30 bg-blue-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-blue-200 disabled:opacity-60"
                    >
