...
import { 
  ...
  ChevronRight, ArrowRight, Eye, EyeOff,
    ...
    <main className={`min-h-screen flex flex-col lg:flex-row ${current.bg} ${current.text}`}>
      ...
      <div className="w-full lg:w-[45%] p-6 md:p-12 lg:p-20 flex flex-col justify-center">
        ...
        >
          ...
          <p className={`${current.subtext} text-sm sm:text-base mb-12`}>Fill in your details to get started with the ultimate driving experience.</p>
          ...
          <form className="space-y-6" onSubmit={handleRegister}>
            ...
            <div className="flex items-center gap-3 py-2">
              ...
              <p className="text-[11px] font-medium opacity-60">I agree to the <span className="text-blue-500 underline cursor-pointer">Terms of Service</span> and <span className="text-blue-500 underline cursor-pointer">Privacy Policy</span>.</p>
            </div>
          ...
          <p className="mt-12 text-center text-sm font-medium opacity-60">
            Already have an account? <Link to="/login" className="text-blue-500 font-bold hover:underline">Sign In <ArrowRight className="w-4 h-4 inline ml-1" /></Link>
          </p>
        </motion.div>
      </div>
      ...
      <div className="hidden lg:flex flex-1 relative bg-[#05070a] border-l border-white/5 p-20 flex-col justify-center items-center text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg">
           ...
           <p className="text-xl text-white/50 mb-16 leading-relaxed">Create your account today and unlock access to our premium fleet. Your next adventure starts here.</p>

           <div className="grid grid-cols-2 gap-6 text-left">
              {features.map((f, i) => (
                <div key={i} className="p-6 rounded-3xl bg-white/5 border border-white/5 backdrop-blur-3xl">
                   <f.icon className="w-6 h-6 text-blue-500 mb-4" />
                   <h3 className="text-sm font-black uppercase tracking-tight mb-1">{f.title}</h3>
