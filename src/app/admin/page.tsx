      ...
      {!loading && !error && data && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              { label: 'Total Users', value: data.stats.totalUsers, sub: `${data.stats.activeUsers} active`, icon: Users },
              { label: 'Total Bookings', value: data.stats.totalBookings, sub: `${data.stats.pendingBookings} pending`, icon: CalendarClock },
              { label: 'Vehicles', value: data.stats.totalVehicles, sub: `${data.stats.pendingListings} awaiting approval`, icon: CarFront },
              { label: 'Pending Bookings', value: data.stats.pendingBookings, sub: 'Needs review', icon: Clock3 },
            ].map((item, idx) => (
              <motion.div
                key={item.label}
          ...
          {data.recentBookings.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              <AdminTable columns={['Booking', 'Customer', 'Vehicle', 'Dates', 'Total', 'Status']} minWidth="min-w-[860px]">
                {data.recentBookings.map((booking) => (
                  <tr key={booking.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-4 text-xs opacity-80">{booking.startDate} to {booking.endDate}</td>
                    <td className="px-5 py-4 font-black text-sm">${booking.total}</td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.18em] ${bookingStatusClass(booking.status)}`}>
                        {booking.status}
                      </span>
                    </td>
                  </tr>
